...
if ( !class_exists( 'WPSL_EXT_UserManaged' ) ) {
	...
	class WPSL_EXT_UserManaged {
		...
		public function init() {
			...
			add_filter('wpsl_store_meta',                        array($this,'filter_wpsl_store_meta'             ), 10,  2   );
			add_filter('wpsl_store_header_template',             array($this,'filter_wpsl_store_header_template'  )           );
		 ...
		 *
		 */
		public static function wpsl_ext_wp_print_styles() {
		 ...
		 *
		 */
		function filter_wpsl_store_meta( $store_meta, $store_ID ) {
			// $this->debugMP('pr',__FUNCTION__.' store_data['. $store_ID .']=', $store_meta);
			...
			if ( $this->wpsl_ext_is_user_location( $store_ID ) ) {
				...
				$store_meta['wpsl_ext_user_location_allowed'] = 1;
			} else {
				// $store_meta['wpsl_ext_store_meta_user_id']    = 'NoUSER' . $store_ID;
				$store_meta['wpsl_ext_user_location_allowed'] = 0;
			}
		 ...
		 *
		 */
		function filter_wpsl_store_header_template( $header_template ) {
		...
		function wpsl_show_user_buttons( $location_id ) {
			...
			if ( $wpsl_ext_options->is_true('ext_uml_show_uml_buttons') ) {
				...
				if ( $user_buttons_string != '' ) {
					$user_buttons_output .= '<div class="wpsl-event-wrap">';
		...
		function filter_manage_users_custom_column( $value, $column_name, $user_id ) {
			...
			switch ( $column_name ) {
				...
				case WPSL_EXT_STORE_USER_COL_ALLOWED:
					...
					if ( $cur_user ) {
						$user_allowed = wpsl_ext_is_user_allowed( $user_id ) ? __( 'Allowed','wp-store-locator-extenders' ) : __( 'Disallowed','wp-store-locator-extenders' );
					}
					return $user_allowed;
					break;
