...
if ( !defined( 'WPSL_EXT_FREEMIUS_ID' ) ) {
    define( 'WPSL_EXT_FREEMIUS_ID', '7247' );
}
//
if ( !defined( 'WPSL_EXT_SHORT_SLUG' ) ) {
    define( 'WPSL_EXT_SHORT_SLUG', 'wp-store-locator-extenders' );
}
//
if ( !defined( 'WPSL_EXT_PREMIUM_SLUG' ) ) {
    define( 'WPSL_EXT_PREMIUM_SLUG', 'wp-store-locator-extenders-premium' );
}
//
if ( !defined( 'WPSL_EXT_CLASS_PREFIX' ) ) {
    define( 'WPSL_EXT_CLASS_PREFIX', 'WPSL_Extenders_' );
}
//
if ( !defined( 'WPSL_EXT_ADMIN_PAGE_SLUG' ) ) {
    define( 'WPSL_EXT_ADMIN_PAGE_SLUG', 'wpsl_ext_admin_menu' );
}
//
if ( !defined( 'WPSL_EXT_ADMIN_PAGE_SLUG_FRE' ) ) {
    define( 'WPSL_EXT_ADMIN_PAGE_SLUG_FRE', 'wpsl_ext_admin_menu-pricing' );
}
//
if ( !defined( 'WPSL_EXT_MIN_WPSL' ) ) {
    define( 'WPSL_EXT_MIN_WPSL', '2.2.0' );
}
//
if ( !defined( 'WPSL_EXT_FILE' ) ) {
    define( 'WPSL_EXT_FILE', __FILE__ );
}
//
if ( !defined( 'WPSL_EXT_REL_DIR' ) ) {
    define( 'WPSL_EXT_REL_DIR', plugin_dir_path( WPSL_EXT_FILE ) );
}
//
if ( !defined( 'WPSL_EXT_BASENAME' ) ) {
    define( 'WPSL_EXT_BASENAME', plugin_basename( __FILE__ ) );
}
if ( !defined( 'WPSL_EXT_PLUGIN_DIR' ) ) {
    define( 'WPSL_EXT_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
}
if ( !defined( 'WPSL_EXT_PLUGIN_URL' ) ) {
    define( 'WPSL_EXT_PLUGIN_URL', plugins_url( '', __FILE__ ) );
}
if ( !defined( 'WPSL_EXT_ADMIN_MENU_SLUG' ) ) {
    define( 'WPSL_EXT_ADMIN_MENU_SLUG', 'wpsl_ext_admin_menu' );
}
if ( !defined( 'WPSL_POST_TYPE' ) ) {
    define( 'WPSL_POST_TYPE', 'wpsl_stores' );
}
if ( !defined( 'WPSL_EXT_OPTION_NAME' ) ) {
    define( 'WPSL_EXT_OPTION_NAME', 'wp-store-locator-extenders-options' );
}
if ( !defined( 'WPSL_EXT_NOTICE_OPTION' ) ) {
    define( 'WPSL_EXT_NOTICE_OPTION', 'wpsl_ext_notices' );
}
if ( !function_exists( 'get_plugin_data' ) ) {
    include_once ABSPATH . 'wp-admin/includes/plugin.php';
}
$this_plugin = get_plugin_data( WPSL_EXT_FILE, false, false );
if ( !defined( 'WPSL_EXT_VERSION_NUM' ) ) {
    define( 'WPSL_EXT_VERSION_NUM', $this_plugin['Version'] );
}
...
if ( function_exists( 'wpsl_ext_freemius' ) ) {
     ...
     * @param string $msg
     */
    // function WPSL_Extenders_debugMP($type='msg', $header='', $message='', $file=null, $line=null, $notime=true ) {
    // $panel='wpsl.ext';
...
{
    $panel = 'wpsl.ext';
    // Only use error_log when WP_DEBUG_LOG_EXT defined as true in wp-config
    if ( defined( 'WP_DEBUG_LOG_EXT' ) && WP_DEBUG_LOG_EXT ) {
        switch ( strtolower( $type ) ) {
            case 'pr':
                error_log( 'HDR: ' . $header . ' PR is no MSG ' . print_r( $message, true ) );
                break;
